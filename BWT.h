...
class BWT {
  protected:
    ...
    PrefixOccurrences4 prefixOccurrences; // the prefix occurences table

    length_t dollarPos; // the position of the dollar in the bwt
    std::vector<length_t> delimitingPos; // the positions of % (word
                                         // delimiting) chars in the text
    ...
    /**
     * Function to check if a range in the text goes over a word delimiting
     * character
     * @param begin, the beginning of the range
     * @param end the end of the range
     * @param delimitPos reference that, if the range goes over a delimiting
     * character will store the position of that delimiting character
     * @returns true if the range goes over a word delimiting character,
     * false otherwise
     */
    bool goesOverWordDelimiting(length_t begin, length_t end,
  ...
  public:
     ...
     */
    BWT(const std::string& prefix) : prefix(prefix) {
    ...
    BWT(const std::string& prefix, int sa_sparse) : prefix(prefix) {
    ...
    std::vector<AppMatch> mapOccurencesInSAToOccurencesInText(
        const std::vector<AppMatchSA>& occurences, length_t maxED,
        const Substring& pattern);
};
